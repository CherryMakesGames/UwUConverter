...
# Inno Setup writes the first line before starting PowerShell. Append here.
...
try {
    if (Test-Path -LiteralPath $CertificateState) {
        $OldThumbprint = (Get-Content -LiteralPath $CertificateState -Raw).Trim()

        if ($OldThumbprint) {
            $OldCertPath = "Cert:\CurrentUser\TrustedPeople\" + $OldThumbprint

            if (Test-Path $OldCertPath) {
                Write-Log ("Removing previous development certificate: " + $OldThumbprint)
                Remove-Item -Path $OldCertPath -Force -ErrorAction SilentlyContinue
            }
        }

        Remove-Item -LiteralPath $CertificateState -Force -ErrorAction SilentlyContinue
    }
}
catch {
    Fail "cleaning previous certificate" $_
}

try {
    # Avoid constructor syntax that can be parsed differently by old Windows
    # PowerShell. New-Object -ArgumentList works on Windows PowerShell 5.1.
    $Certificate = New-Object `
        -TypeName System.Security.Cryptography.X509Certificates.X509Certificate2 `
        -ArgumentList $CertificatePath

    Write-Log ("Certificate subject: " + $Certificate.Subject)
    Write-Log ("Certificate issuer: " + $Certificate.Issuer)
    Write-Log ("Certificate thumbprint: " + $Certificate.Thumbprint)

    if ($Certificate.Subject -eq $Certificate.Issuer) {
        Write-Log "Self-signed development certificate detected."
        Write-Log "Importing certificate to CurrentUser\TrustedPeople."
        ...
        if (!$TrustedCertificate) {
            throw "Certificate import returned successfully, but the certificate is not present in CurrentUser\TrustedPeople."
        }

        Write-Log "Certificate is present in CurrentUser\TrustedPeople."
        ...
        Write-Log "Certificate is not self-signed. Windows will validate its normal trust chain."
    }
}
catch {
